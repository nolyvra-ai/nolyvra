...
export default function PricingPage() {
  ...
  const CARD_W = { minWidth:220, width:220, flexShrink:0 };
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",color:"#0F1623",background:"#fff",overflowX:"hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .plan-card{background:#fff;border:1px solid #E2E6ED;border-radius:14px;padding:24px 18px;display:flex;flex-direction:column;position:relative;transition:box-shadow .2s,transform .2s;box-shadow:0 2px 8px rgba(0,0,0,.05)}
        ...
        .scroll-arrow-btn{width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.95);border:1px solid #E2E6ED;cursor:pointer;font-size:22px;color:#3D4A63;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.08);transition:all .15s;flex-shrink:0;font-family:inherit;line-height:1;padding:0}
        .scroll-arrow-btn:hover{box-shadow:0 4px 16px rgba(0,0,0,.15);background:#fff;transform:scale(1.05)}
      `}</style>
      ...
      <div style={{ padding:"0 48px 96px",background:"#F7F8FA" }}>
        <div style={{ maxWidth:1340,margin:"0 auto",display:"flex",alignItems:"center",gap:10 }}>
          ...
          <button className="scroll-arrow-btn" onClick={() => scrollRef.current?.scrollBy({left:-250,behavior:"smooth"})}>‹</button>

          <div ref={scrollRef} className="hide-scrollbar" style={{ flex:1,display:"flex",gap:14,overflowX:"auto",scrollBehavior:"smooth",paddingTop:30,paddingBottom:16 }}>
            ...
            <div className="plan-card" style={CARD_W}>
              ...
              <div style={{ fontSize:17,fontWeight:700,color:"#0F1623",marginBottom:5 }}>Free Trial</div>
            ...
            <div className="plan-card" style={CARD_W}>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:9,flex:1,marginBottom:22 }}>
                ...
                {featItem("Placement Probability")}
                {featItem("AI Email Generator")}
                {featItem("Top-up tokens anytime")}
                {featItem("AI Talent Search",null,true)}
                {featItem("Advanced AI Model",null,true)}
              </div>
              <button className="pricing-btn" onClick={() => handleSubscribe("plan-payg")} disabled={subscribing === "plan-payg"} style={{ background:"#059669",color:"#fff",opacity:subscribing==="plan-payg"?0.7:1 }}>{subscribing === "plan-payg" ? "Redirecting…" : "Get Started"}</button>
            </div>
            ...
            <div className="plan-card" style={CARD_W}>
              ...
              <div style={{ fontSize:17,fontWeight:700,color:"#0F1623",marginBottom:5 }}>Solopreneur</div>
            ...
            {/* SILVER — FEATURED */}
            <div className="plan-card featured" style={CARD_W}>
              ...
              <div style={{ fontSize:17,fontWeight:700,color:"#0F1623",marginBottom:5 }}>Small Enterprise</div>
            ...
            <div className="plan-card" style={CARD_W}>
              ...
              <div style={{ fontSize:17,fontWeight:700,color:"#0F1623",marginBottom:5 }}>Medium Enterprise</div>
            ...
            <div className="plan-card" style={CARD_W}>
              ...
              <div style={{ fontSize:17,fontWeight:700,color:"#0F1623",marginBottom:5 }}>Large Enterprise</div>
        ...
        <div style={{ maxWidth:1200,margin:"0 auto" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20 }}>
            {[["🪙","What is a Token?","Tokens power AI analysis. Each candidate analysis consumes tokens for CV scoring, authenticity detection, interview question generation and placement probability. 1 full analysis ≈ 10 tokens."],
              ["🔄","Do tokens roll over?","Yes — unused tokens roll over to the next month, so you never lose what you've paid for. Your balance accumulates until you use it or upgrade your plan."],
          ...
          <table className="compare-table">
            ...
            <tbody>
              {[["Candidates / month","3","30","150","750","2,250","Unlimited"],
                ...
                <tr key={feat}>
                  <td>{feat}</td>
                  {vals.map((v,i) => <td key={i} style={i===2||i===3?{background:"rgba(29,114,232,.04)"}:{}}>{v}</td>)}
                </tr>
              ))}
              {[["AI Candidate Analysis",1,1,1,1,1,1],
                ["CV vs LinkedIn Scoring",1,1,1,1,1,1],
                ["Risk Flag Detection",1,1,1,1,1,1],
                ["Authenticity Detection",1,1,1,1,1,1],
                ["Placement Probability",1,1,1,1,1,1],
                ["AI Email Generator",1,1,1,1,1,1],
                ["AI Talent Search",0,0,1,1,1,1],
                ["Interview Scheduler",0,0,1,1,1,1],
                ["Client Brief Analyzer",0,0,1,1,1,1],
                ["Advanced AI Model",0,0,0,1,1,1],
                ["Email Integration",0,0,0,1,1,1],
                ["Video Interview",0,0,0,0,1,1],
                ["Advanced Analytics",0,0,0,0,1,1],
                ["Custom Integrations",0,0,0,0,0,1],
                ["SLA Guarantee",0,0,0,0,0,1],
              ].map(([feat,...vals]) => (
                <tr key={feat}>
                  <td>{feat}</td>
                  {vals.map((v,i) => (
                    <td key={i} style={i===2||i===3?{background:"rgba(29,114,232,.04)"}:{}}>
