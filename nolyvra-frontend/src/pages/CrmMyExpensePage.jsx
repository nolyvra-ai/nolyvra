...
function NewExpenseDialog({ open, onClose, loginId, employeeId, onCreated }) {
  ...
  function reset() {
    setTitle(""); setAmount(""); setCategory(""); setExpenseDate("");
    setNotes(""); setReceipt(null); setErr("");
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/crm/employees/${employeeId}/expenses?loginId=${loginId}`,
        { method: "POST", headers: authH(), body: form }
      );
      if (!res.ok) {
        const text = await res.text();
        try { const j = JSON.parse(text); setErr(j.message || j.error || "Failed to submit expense."); }
        catch { setErr(text || "Failed to submit expense."); }
        return;
      }
  ...
  return (
    <Dialog open={open} onClose={() => { reset(); onClose(); }} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontSize: 15, fontWeight: 700, pb: 1 }}>New Expense</DialogTitle>
      <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 2, pt: "8px !important" }}>
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          ...
          <FormControl size="small" fullWidth>
            ...
            <Select label="Category" value={category} onChange={e => setCategory(e.target.value)} sx={{ fontSize: 12 }}>
              {CATEGORIES.map(c => <MenuItem key={c} value={c} sx={{ fontSize: 12 }}>{c}</MenuItem>)}
            </Select>
          </FormControl>
        </Box>
        <TextField size="small" label="Expense date" type="date" value={expenseDate}
          onChange={e => setExpenseDate(e.target.value)} fullWidth
          InputLabelProps={{ shrink: true }} sx={{ "& .MuiInputBase-input": { fontSize: 12 } }} />
        <TextField size="small" label="Notes (optional)" value={notes}
        ...
        <Button component="label" size="small" variant="outlined" sx={{
          ...
          <input type="file" hidden onChange={e => setReceipt(e.target.files?.[0] || null)} />
        </Button>
      </DialogContent>
...
export default function CrmMyExpensePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/crm/employees/${employeeId}/expenses?loginId=${loginId}`, { headers: authH() });
  ...
  async function cancel(id) {
    if (!window.confirm("Cancel this expense submission?")) return;
    try {
      const res = await fetch(`${API_BASE}/api/crm/expenses/${id}?loginId=${loginId}`,
  ...
  const pendingCount = expenses.filter(e => e.status === "IN_PROGRESS").length;
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2.5 }}>
        <Box>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            <Typography sx={{ fontSize: 20, fontWeight: 800, color: TEXT }}>My Expenses</Typography>
            {pendingCount > 0 && (
              <Box sx={{
          ...
          <Typography sx={{ fontSize: 12.5, color: MUTED, mt: 0.25 }}>
            Submit expenses and track their status
          </Typography>
        </Box>
      ...
      {loading ? (
        ...
        <Paper elevation={0} sx={{ ...CARD, overflow: "hidden" }}>
          <Table size="small">
            ...
            <TableBody>
              {expenses.map(exp => (
                <TableRow key={exp.id} sx={{ "&:last-child td": { borderBottom: 0 } }}>
                  ...
                  <TableCell sx={{ py: 1.5, px: 2, borderBottom: `1px solid ${BORDER}`, textAlign: "right" }}>
                    {exp.status === "IN_PROGRESS" && (
                      <Tooltip title="Cancel expense">
                        <IconButton size="small" onClick={() => cancel(exp.id)}
